    ...
    it 'calls chat.postMessage Slack API with correct parameters' do
      stub_request(:post, 'https://slack.com/api/chat.postMessage')
        .with(body: "{\"channel\":\"#{channel}\",\"text\":\"#{message}\"}",
              headers: { Authorization: "Bearer #{slack_bot_access_token}",
                         'Content-Type': 'application/json; charset=utf-8', Host: 'slack.com' })
        .to_return(status: 200, body: '', headers: {})

      expect(described_class.post_message(slack_bot_access_token, channel, message).code).to eq('200')
    end
  end
end
